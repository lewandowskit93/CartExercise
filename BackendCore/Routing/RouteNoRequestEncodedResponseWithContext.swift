//
//  RouteNoRequestEncodedResponseWithContext.swift
//  CartExercise
//
...
public struct RouteNoRequestEncodedResponseWithContext<Response: Encodable>: PRoute {
    ...
    private let handler: (RequestContext) async throws -> Response
    ...
    public init(
        ...
        handler: nonisolated(nonsending) @escaping (RequestContext) async throws -> Response)
    {
        self.path = path
        self.method = method
    ...
    public func handle(_ context: RequestContext) async throws -> HttpResponse {
        ...
        let payload = try await handler(context)
